...
// Fix the syntax errors in addPdfSlot function
// The issue is single quotes inside string concatenation — needs \' or "
const fs = require('fs');
...
// Find and replace the broken addPdfSlot function
const badStart = 'window.Views.addPdfSlot = function(modalType) {';
const badEnd = '  if (window.lucide) window.lucide.createIcons({ nodes: [slot] });\r\n};';

const startIdx = c.indexOf(badStart);
const endIdx = c.indexOf(badEnd, startIdx);
console.log('addPdfSlot: start=', startIdx, 'end=', endIdx + badEnd.length);

if (startIdx === -1 || endIdx === -1) {
  console.error('Could not find addPdfSlot to fix!');
  process.exit(1);
}
...
const fixedAddPdfSlot = `window.Views.addPdfSlot = function(modalType) {
  ...
  // Build each part separately to avoid quote conflicts
  var mType = JSON.stringify(modalType); // e.g. '"add"'
  ...
  var htmlRemoveBtn = '<button type="button" ' +
    'onclick="window.Views.removePdfSlot(' + mType + ',' + idx + ')" ' +
  ...
  var htmlFileInput = '<input type="file" id="' + modalType + '-pdf-file-' + idx + '" ' +
    'accept="application/pdf" ' +
    'onchange="window.Views.handleMultiPdfUpload(' + mType + ',' + idx + ',this)" ' +
    'class="hidden">';

  var htmlUploadBtn = '<button type="button" ' +
    'onclick="document.getElementById(\'' + modalType + '-pdf-file-' + idx + '\').click()" ' +
...
c = c.substring(0, startIdx) + fixedAddPdfSlot + c.substring(endIdx + badEnd.length);
fs.writeFileSync(featPath, c, 'utf8');
console.log('✅ addPdfSlot fixed!');
