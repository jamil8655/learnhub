  ...
  container.innerHTML = `
    <div class="space-y-5 font-urdu max-w-7xl mx-auto px-3 sm:px-6 py-4 select-none" dir="rtl">
      ...
      <div class="bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-slate-800 p-2 rounded-2xl shadow-sm overflow-x-auto scrollbar-none flex items-center gap-1.5" style="-webkit-overflow-scrolling: touch;">
        ...
        <a href="#/admin/users" class="py-2 px-3.5 rounded-xl font-bold text-xs shrink-0 flex items-center gap-1.5 transition bg-teal-700 text-white shadow-sm">
      ...
      <div class="bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-slate-800 p-4 sm:p-6 rounded-3xl text-slate-900 dark:text-white shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div class="space-y-1">
          <div class="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-blue-50 dark:bg-blue-950/60 text-blue-800 dark:text-blue-300 border border-blue-600/30 text-[10px] font-bold">
            <i data-lucide="shield" class="w-3.5 h-3.5 text-blue-600"></i>
            <span>صارفین اور سیکیورٹی ڈائریکٹری (RBAC Suite)</span>
          </div>
          <h1 class="text-lg sm:text-2xl font-black text-slate-900 dark:text-white">
            صارفین کا انتظامی کنٹرول اور رسائی
          </h1>
          <p class="text-xs text-slate-500 dark:text-slate-400 max-w-2xl leading-relaxed">
            اکاؤنٹس کا انتظام، کردار تفویض، سیکیورٹی آڈٹ، 2FA معائنہ اور سیشنز کو منسوخ کریں۔
          </p>
        </div>

        <button 
          onclick="window.Views.admin.openAddUserModal()" 
          class="py-2.5 px-4 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-bold text-xs shadow-sm flex items-center gap-1.5 shrink-0 transition active:scale-95"
        >
