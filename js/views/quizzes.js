...
window.Views.renderEmbeddedAdventureGame = function() {
  const container = document.getElementById('embedded-adventure-container');
  if (!container) return;
  ...
  window._activeAdventureWorldId = window._activeAdventureWorldId || p.unlockedWorlds[p.unlockedWorlds.length - 1] || 'cls-1';
  const currentClass = worlds.find(w => w.id === window._activeAdventureWorldId) || worlds[0] || {
    id: 'cls-1',
    worldNumber: 1,
    title: 'کلاس 1 — ابتدائی دینی ایڈونچر',
    subtitle: 'بنیادی حروف، کلمۂ طیبہ، دعائیں اور اچھے اخلاق'
  };
  ...
  container.innerHTML = `
    <div class="space-y-6 ${fontClass}" dir="${isRtl ? 'rtl' : 'ltr'}">
      ...
      <div class="p-4 sm:p-5 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-slate-800 shadow-sm flex flex-wrap items-center justify-between gap-4">
        ...
        <div class="flex items-center gap-3">
          <div class="w-12 h-12 rounded-2xl bg-teal-700 text-white flex items-center justify-center font-bold text-sm shadow-sm">
            Lvl ${p.level}
          </div>
          <div>
            <div class="text-xs font-bold text-slate-800 dark:text-slate-200">
              علمی ترقی: ${p.totalXp} XP (${xpInfo.percentage}%)
            </div>
            <div class="w-32 sm:w-44 h-2 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden mt-1.5 border border-slate-200 dark:border-slate-700">
        ...
        <div class="flex items-center gap-2 flex-wrap text-xs font-bold font-mono">
          <div class="flex items-center gap-1 bg-rose-50 dark:bg-rose-950/50 text-rose-700 dark:text-rose-300 border border-rose-200 dark:border-rose-800/60 px-3 py-1.5 rounded-xl">
            <span>❤️</span>
            <span>${p.hearts}/${p.maxHearts || 5}</span>
          </div>
          <div class="flex items-center gap-1 bg-amber-50 dark:bg-amber-950/50 text-amber-700 dark:text-amber-300 border border-amber-200 dark:border-amber-800/60 px-3 py-1.5 rounded-xl">
            <span>🪙</span>
            <span>${p.coins || 0}</span>
          </div>
          <div class="flex items-center gap-1 bg-orange-50 dark:bg-orange-950/50 text-orange-700 dark:text-orange-300 border border-orange-200 dark:border-orange-800/60 px-3 py-1.5 rounded-xl">
            <span>🔥</span>
      ...
      <!-- 2. REALM / CLASS SELECTOR BAR -->
      <div class="space-y-2">
        <div class="text-xs font-bold text-slate-500 flex items-center justify-between">
          <span>کلاس و جہان کا انتخاب کریں (Class / Realm Selection):</span>
          <span class="text-teal-700 dark:text-teal-400 font-mono">${worlds.length} کلاسز دستیاب</span>
        </div>
        <div class="flex items-center gap-2 overflow-x-auto scrollbar-none pb-1" style="-webkit-overflow-scrolling: touch;">
          ${worlds.map(w => {
            const isUnlocked = p.unlockedWorlds.includes(w.id) || w.id === 'cls-1';
            const isCurrent = w.id === currentClass.id;
            return `
              <button 
                onclick="window.Views.selectAdventureWorld('${w.id}')"
                class="px-4 py-2.5 rounded-2xl text-xs font-bold transition shrink-0 flex items-center gap-2 ${isCurrent ? 'bg-teal-700 text-white shadow-sm' : (isUnlocked ? 'bg-white dark:bg-slate-900 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-800 hover:bg-slate-100' : 'bg-slate-100 dark:bg-slate-800 text-slate-400 opacity-60 border border-slate-200 dark:border-slate-800 cursor-not-allowed')}"
                ${!isUnlocked ? 'disabled' : ''}
              >
      ...
      <!-- 3. ACTIVE CLASS HEADER & TIER SEGMENTS -->
      <div class="p-5 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-slate-800 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <h2 class="text-lg font-bold text-slate-900 dark:text-white mt-1">${currentClass.subtitle || 'دینی علوم و اخلاقی مراحل'}</h2>
        </div>

        <!-- Tier Switcher (1-25, 26-50, 51-75, 76-100) -->
        <div class="flex items-center gap-1 bg-slate-100 dark:bg-slate-800 p-1 rounded-2xl border border-slate-200 dark:border-slate-700 text-xs font-bold font-mono">
          <button onclick="window.Views.selectStageTier(1)" class="px-3 py-1.5 rounded-xl transition ${window._activeStageTier === 1 ? 'bg-teal-700 text-white shadow-sm' : 'text-slate-600 dark:text-slate-400 hover:text-slate-900'}">1-25</button>
          <button onclick="window.Views.selectStageTier(2)" class="px-3 py-1.5 rounded-xl transition ${window._activeStageTier === 2 ? 'bg-teal-700 text-white shadow-sm' : 'text-slate-600 dark:text-slate-400 hover:text-slate-900'}">26-50</button>
          <button onclick="window.Views.selectStageTier(3)" class="px-3 py-1.5 rounded-xl transition ${window._activeStageTier === 3 ? 'bg-teal-700 text-white shadow-sm' : 'text-slate-600 dark:text-slate-400 hover:text-slate-900'}">51-75</button>
          <button onclick="window.Views.selectStageTier(4)" class="px-3 py-1.5 rounded-xl transition ${window._activeStageTier === 4 ? 'bg-teal-700 text-white shadow-sm' : 'text-slate-600 dark:text-slate-400 hover:text-slate-900'}">76-100</button>
        </div>
      </div>

      <!-- 4. MODERN STAGES GRID -->
      <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3.5 sm:gap-4">
        ${displayedLevels.map((s, idx) => {
          ...
          const isUnlocked = s.stageNumber === 1 || isCompleted || p.completedStages.includes(displayedLevels[idx - 1]?.id);
          const stageStars = p.stageStars && p.stageStars[s.id] ? p.stageStars[s.id] : 0;
          const starsDisplay = stageStars === 3 ? '⭐⭐⭐' : (stageStars === 2 ? '⭐⭐☆' : (stageStars === 1 ? '⭐☆☆' : '☆☆☆'));

          return `
            <div class="p-4 rounded-2xl bg-white dark:bg-slate-900 border ${isCompleted ? 'border-emerald-300 dark:border-emerald-800/60 bg-emerald-50/10' : (isUnlocked ? 'border-teal-500 shadow-sm' : 'border-slate-200 dark:border-slate-800 opacity-60')} flex flex-col justify-between space-y-3 transition hover:shadow-md">
