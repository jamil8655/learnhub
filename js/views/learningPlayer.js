...
window.Views.renderLearningPlayer = async function(params) {
  ...
  if (!currentUser) {
    window.App.showToast(t('loginPrompt', 'Please login to access the learning player.'), 'warning');
  ...
  if (!course || !course.lessons || course.lessons.length === 0) {
    container.innerHTML = `
      <div class="max-w-3xl mx-auto px-4 py-20 text-center space-y-4 ${fontClass}" dir="${isRtl ? 'rtl' : 'ltr'}">
        ...
        <h2 class="text-2xl font-extrabold text-slate-900 dark:text-white">${t('noCoursesFound', isRtl ? 'کوئی اسباق دستیاب نہیں ہیں' : 'No Lessons Available')}</h2>
        <p class="text-slate-500 dark:text-slate-400 text-sm">${t('noCoursesSubtitle', isRtl ? 'اس کورس میں فی الحال تدریسی مواد اپلوڈ نہیں کیا گیا ہے۔' : 'Educational material is not currently available for this course.')}</p>
        <a href="#/courses" class="btn-primary py-2.5 px-6 text-xs inline-flex items-center gap-2">
          ...
          <span>${t('viewAllCourses', isRtl ? 'کورسز کی فہرست پر واپس جائیں' : 'Back to Courses')}</span>
        </a>
      </div>
    `;
  ...
  const defaultAuthor = lang === 'en' ? 'Tariq Mahmood' : (lang === 'ar' ? 'محمد عثمان' : 'محمد عثمان');
  const defaultQuestion = lang === 'en' ? 'Does this rule apply uniformly across all classical qiraat?' : (lang === 'ar' ? 'يا شيخ! هل تنطبق هذه القاعدة بالتساوي في جميع القراءات السبع؟' : 'استاد محترم! کیا اس قاعدے کا اطلاق تمام قراءتوں میں یکساں ہے؟');
  const defaultInstructor = lang === 'en' ? 'Shaykh Abdur-Rahman (Course Instructor)' : (lang === 'ar' ? 'الشيخ عبد الرحمن (مدرس الدورة)' : 'مفتی عبدالرحمن (استادِ کورس)');
  const defaultAnswer = lang === 'en' ? 'Yes, the fundamental principle is identical in Hafs and the seven authentic qiraat.' : (lang === 'ar' ? 'نعم، الأصل والضابط واحد في رواية حفص وعامة القراءات المتواترة.' : 'جی ہاں، حفص عن عاصم اور اکثر قراءتِ سبعہ میں اس کا بنیادی اصول یکساں ہے۔');
  ...
  const lessonQuestions = JSON.parse(localStorage.getItem(qaStorageKey) || JSON.stringify([
    {
      ...
      timestamp: isRtl ? '2 گھنٹے پہلے' : '2 hours ago',
      votes: 5,
      answer: {
        author: defaultInstructor,
        badge: t('certifiedInstructorBadge', isRtl ? 'مستند استاد' : 'Certified Instructor'),
        text: defaultAnswer,
        timestamp: isRtl ? '1 گھنٹہ پہلے' : '1 hour ago'
      }
    }
  ]));
  ...
  container.innerHTML = `
    ...
    <div class="bg-slate-900/95 backdrop-blur text-white border-b border-slate-800 px-4 sm:px-6 py-3 flex items-center justify-between sticky top-0 z-30 shadow-lg ${fontClass}" dir="${isRtl ? 'rtl' : 'ltr'}">
      <div class="flex items-center gap-3 min-w-0 flex-1">
        <a href="#/courses/${course.id}" class="text-slate-400 hover:text-emerald-400 p-2 rounded-xl hover:bg-slate-800 transition shrink-0" title="${t('backToCourseTooltip', isRtl ? 'کورس کے صفحہ پر واپس جائیں' : 'Back to Course')}">
        ...
        <div class="min-w-0 flex-1">
          ...
          <div class="text-sm sm:text-base font-extrabold text-white truncate flex items-center gap-2">
            ...
            ${isCompleted ? `<span class="badge bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 text-[10px] py-0.5 px-2">${t('completedBadge', isRtl ? 'مکمل شدہ ✓' : 'Completed ✓')}</span>` : ''}
          </div>
        </div>
      </div>
      ...
      <div class="flex items-center gap-3 sm:gap-5 shrink-0">
        ...
        <div class="hidden md:flex items-center gap-3 bg-slate-800/80 py-1.5 px-3.5 rounded-xl border border-slate-700/60">
          <div class="${isRtl ? 'text-right' : 'text-left'}">
            <div class="text-[10px] text-slate-400">${t('courseProgressLabel', isRtl ? 'کورس کی پیش رفت' : 'Course Progress')}</div>
            <div class="text-xs font-extrabold text-amber-400">${progressPercent}% ${t('completedPercentage', isRtl ? 'مکمل' : 'Complete')}</div>
          </div>
        ...
        ${progressPercent === 100 ? `
          <button onclick="window.Views.openRoyalCertificateModal('${course.id}')" class="btn-primary py-1.5 px-3.5 text-xs bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-600 hover:to-amber-700 text-slate-950 font-extrabold border-none shadow-lg shadow-amber-500/20 flex items-center gap-1.5 animate-pulse">
            ...
            <span>${t('getRoyalCertificateBtn', isRtl ? 'شاہی سند حاصل کریں 🎓' : 'Claim Certificate 🎓')}</span>
          </button>
        ` : ''}
    ...
    <div class="flex flex-col lg:flex-row min-h-[calc(100vh-130px)] bg-slate-950 ${fontClass}" dir="${isRtl ? 'rtl' : 'ltr'}">
      ...
      <div class="w-full lg:w-[70%] flex-1 flex flex-col justify-between overflow-y-auto">
        <div class="p-3 sm:p-6 lg:p-8 max-w-5xl mx-auto w-full space-y-6">
          ...
          <div class="bg-black rounded-3xl overflow-hidden shadow-2xl border border-slate-800 relative group">
            ...
            ` : (activeLesson.type === 'audio' || activeLesson.audioUrl) ? `
              <div class="p-8 sm:p-12 text-center bg-gradient-to-br from-slate-900 via-emerald-950/60 to-slate-950 flex flex-col items-center justify-center space-y-6">
                ...
                <div class="max-w-xl">
                  <span class="badge bg-emerald-500/20 text-emerald-300 text-[10px] font-bold uppercase tracking-wider mb-2 inline-block">${t('audioListen', isRtl ? 'صوتی درس و تلاوت (Audio Lecture)' : 'Audio Lecture')}</span>
                  <h3 class="text-xl sm:text-2xl font-bold text-white mb-2">${activeLesson.title}</h3>
                  <p class="text-xs text-emerald-400/90 font-mono">${activeLesson.sectionTitle || t('navCourses', 'General')} • ${t('durationLabel', isRtl ? 'دورانیہ:' : 'Duration:')} ${activeLesson.durationMinutes || '15'} ${t('durationLabel', isRtl ? 'منٹ' : 'min')}</p>
                </div>
                ...
                <div class="w-full max-w-xl bg-slate-900/90 rounded-3xl p-5 border border-emerald-500/30 shadow-2xl space-y-4">
                  ...
                  <div class="flex items-center justify-between text-xs text-slate-400 pt-2 border-t border-slate-800" dir="ltr">
                    <div class="flex items-center gap-2">
                      <button type="button" onclick="document.getElementById('main-lesson-audio').currentTime -= 15" class="p-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-200" title="-15s">-15s</button>
                      <button type="button" onclick="document.getElementById('main-lesson-audio').currentTime += 15" class="p-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-200" title="+15s">+15s</button>
                    </div>
            ...
            ` : (activeLesson.type === 'image' || activeLesson.imageUrl) ? `
              <div class="p-4 sm:p-8 bg-slate-900 flex flex-col items-center justify-center space-y-4">
                ...
                <p class="text-xs text-slate-400">${activeLesson.title} — ${t('topicsAndSciences', isRtl ? 'تفصیلی انفوگرافک و ڈایاگرام' : 'Visual Diagram & Infographic')}</p>
              </div>
            ` : `
              <div class="p-6 sm:p-12 bg-slate-900 text-white rounded-3xl space-y-6 border border-slate-800">
                <div class="flex items-center gap-2 text-xs text-emerald-400 font-bold uppercase tracking-wider">
                  ...
                  <span>${t('textReadingBadge', isRtl ? 'تحریری مطالعہ کا سبق (Reading Module)' : 'Reading Module')}</span>
                </div>
            ...
            ${activeLesson.arabicText ? `
              <div class="p-5 sm:p-6 bg-gradient-to-l from-amber-950/40 via-slate-900 to-amber-950/40 border-t border-amber-500/30 text-center font-arabic text-amber-200 text-lg sm:text-xl leading-loose">
                <div class="text-[11px] font-urdu text-amber-400/80 mb-1">📖 ${t('quranTitle', isRtl ? 'قرآنی آیت / حدیث شریف مع اعراب:' : 'Ayah / Hadith:')}</div>
          ...
          ${activeLesson.contentBody ? `
            <div class="p-5 sm:p-6 rounded-3xl bg-slate-900 border border-slate-800 text-slate-200 space-y-3 ${fontClass} ${isRtl ? 'text-right' : 'text-left'}" dir="${isRtl ? 'rtl' : 'ltr'}">
              <h4 class="text-sm font-extrabold text-emerald-400 flex items-center gap-2">
                <i data-lucide="file-text" class="w-4 h-4"></i>
                <span>${t('courseDescriptionTitle', isRtl ? 'خلاصۂ درس و تفصیلی شرعی تشریح:' : 'Lesson Overview & Detailed Analysis:')}</span>
              </h4>
          ...
          ${(activeLesson.attachments && activeLesson.attachments.length > 0) ? `
            <div class="p-4 sm:p-5 rounded-2xl bg-amber-950/20 border border-amber-500/30 space-y-3 ${fontClass} ${isRtl ? 'text-right' : 'text-left'}" dir="${isRtl ? 'rtl' : 'ltr'}">
              <div class="flex items-center justify-between">
                <span class="text-xs font-bold text-amber-300 flex items-center gap-2">
                  <i data-lucide="folder-down" class="w-4 h-4 text-amber-400"></i> ${t('downloadableResourcesTitle', isRtl ? 'نصابی کتب و پی ڈی ایف فائلز' : 'Course Files & PDFs')} (${activeLesson.attachments.length})
                </span>
                <span class="text-[10px] text-slate-400">${t('downloadBtn', isRtl ? 'ڈاؤن لوڈ کے لیے دستیاب' : 'Available for download')}</span>
              </div>
                ...
                ${activeLesson.attachments.map(att => `
                  <a href="${att.url}" target="_blank" download class="p-3 bg-slate-900 rounded-xl border border-amber-500/20 hover:border-amber-400 flex items-center justify-between gap-3 text-xs transition group">
                    <span class="font-bold text-slate-200 group-hover:text-amber-300 flex items-center gap-2 truncate">
                      ...
                      <span class="truncate">${att.title || t('resourcesTab', 'Document')}</span>
                    </span>
                    <span class="badge bg-amber-500/20 text-amber-300 text-[10px] shrink-0 font-mono flex items-center gap-1">
                      <i data-lucide="download" class="w-3 h-3"></i> ${t('downloadBtn', isRtl ? 'محفوظ کریں' : 'Download')}
                    </span>
                  </a>
                `).join('')}
              </div>
          ...
          <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-900/90 p-4 sm:p-5 rounded-2xl border border-slate-800 text-white shadow-lg">
            <div>
              <h3 class="text-base sm:text-lg font-extrabold text-white flex items-center gap-2">
                ...
                <span class="text-xs px-2.5 py-0.5 rounded-full bg-slate-800 text-slate-400 font-normal">${t('courseLessons', isRtl ? 'سبق' : 'Lesson')} #${currentIndex + 1}</span>
              </h3>
              <p class="text-xs text-slate-400 mt-1">${activeLesson.description || t('timestampNotesSubtitle', isRtl ? 'اس سبق کو دیکھ کر اپنے نوٹس درج کریں اور مارک کریں تاکہ آپ کی پیش رفت محفوظ ہو۔' : 'Watch the lesson, take notes, and mark complete to record your progress.')}</p>
            </div>
            ...
            <div class="flex flex-wrap items-center gap-2 sm:gap-3 shrink-0">
              <button 
                ...
                <span>${isCompleted ? t('lessonCompletedBtn', isRtl ? 'سبق مکمل ہو گیا ✓' : 'Lesson Completed ✓') : t('markCompleteBtn', isRtl ? 'مکمل قرار دیں' : 'Mark as Complete')}</span>
              </button>
              ...
              <button onclick="window.Views.openTimestampNoteModal('${courseId}', '${activeLesson.id}', '${activeLesson.title}')" class="btn-secondary py-2.5 px-3.5 text-xs bg-slate-800 hover:bg-slate-700 text-amber-400 border border-amber-500/30 rounded-xl flex items-center gap-1.5 shadow-sm">
                ...
                <span>${t('writeNoteBtn', isRtl ? '+ نوٹ لکھیں' : '+ Add Note')}</span>
              </button>
            </div>
          </div>
          ...
          <div class="bg-slate-900 rounded-3xl border border-slate-800 overflow-hidden shadow-xl">
            ...
            <div class="flex items-center border-b border-slate-800 overflow-x-auto no-scrollbar bg-slate-950/50">
              <button onclick="window.Views.switchLmsTab('notes')" id="lms-tab-btn-notes" class="lms-tab-btn py-3.5 px-5 text-xs font-bold border-b-2 border-emerald-500 text-emerald-400 flex items-center gap-2 shrink-0">
                <i data-lucide="file-text" class="w-4 h-4"></i>
                <span>${t('personalNotesTab', isRtl ? 'ذاتی نوٹس' : 'Personal Notes')} (${currentLessonNotes.length})</span>
              </button>
              <button onclick="window.Views.switchLmsTab('qa')" id="lms-tab-btn-qa" class="lms-tab-btn py-3.5 px-5 text-xs font-bold border-b-2 border-transparent text-slate-400 hover:text-white flex items-center gap-2 shrink-0">
                ...
                <span>${t('qaDiscussionTab', isRtl ? 'سوال و جواب و ڈسکشن' : 'Q&A Discussion')} (${lessonQuestions.length})</span>
              </button>
              <button onclick="window.Views.switchLmsTab('resources')" id="lms-tab-btn-resources" class="lms-tab-btn py-3.5 px-5 text-xs font-bold border-b-2 border-transparent text-slate-400 hover:text-white flex items-center gap-2 shrink-0">
                ...
                <span>${t('resourcesTab', isRtl ? 'متعلقہ پی ڈی ایف و کتب' : 'Resources & Books')} (${(activeLesson.resources || []).length || 1})</span>
              </button>
            </div>
              ...
              <div class="flex items-center justify-between">
                <div>
                  <h4 class="text-sm font-extrabold text-white flex items-center gap-2">
                    ...
                    <span>${t('timestampNotesTitle', isRtl ? 'اس سبق کے ٹائم اسٹیمپ نوٹس' : 'Timestamped Lesson Notes')}</span>
                  </h4>
                  <p class="text-xs text-slate-400">${t('timestampNotesSubtitle', isRtl ? 'اپنے اہم نکات لکھیں، ٹائم پر کلک کرنے سے ویڈیو وہیں سے چلے گی۔' : 'Take personal notes with timestamps to quickly seek video playback.')}</p>
                </div>
                ${currentLessonNotes.length > 0 ? `
                  <button onclick="window.Views.exportNotesTxt('${course.title}', '${activeLesson.title}')" class="text-xs text-emerald-400 hover:underline flex items-center gap-1">
                    ...
                    <span>${t('exportNotesBtn', isRtl ? 'ایکسپورٹ نوٹس' : 'Export Notes')}</span>
                  </button>
                ` : ''}
              </div>
              ...
              <div class="space-y-2.5">
                ${currentLessonNotes.length === 0 ? `
                  <div class="p-6 text-center border border-dashed border-slate-800 rounded-2xl bg-slate-950/40">
                    ...
                    <p class="text-xs text-slate-400">${t('emptyNotesMsg', isRtl ? 'آپ نے ابھی اس سبق میں کوئی نوٹ شامل نہیں کیا۔' : 'You have not added any notes for this lesson yet.')}</p>
                    <button onclick="window.Views.openTimestampNoteModal('${courseId}', '${activeLesson.id}', '${activeLesson.title}')" class="mt-3 btn-primary py-1.5 px-4 text-xs inline-flex items-center gap-1.5">
                      ...
                      <span>${t('addFirstNoteBtn', isRtl ? '+ پہلا نوٹ شامل کریں' : '+ Add First Note')}</span>
                    </button>
                  </div>
                ` : currentLessonNotes.map((n, idx) => `
                  <div class="p-3.5 bg-slate-950/80 rounded-2xl border border-slate-800 flex items-start justify-between gap-3 group hover:border-emerald-500/40 transition">
                    ...
                    <button onclick="window.Views.deleteTimestampNote('${courseId}', '${n.id}')" class="text-slate-500 hover:text-red-400 p-1 opacity-60 group-hover:opacity-100 transition" title="${t('btnDelete', 'Delete')}">
            ...
            <div id="lms-tab-content-qa" class="p-5 sm:p-6 space-y-5 hidden">
              <div class="flex items-center justify-between">
                <div>
                  <h4 class="text-sm font-extrabold text-white">${t('qaForumTitle', isRtl ? 'استاد و طلباء سے سوال و جواب' : 'Lesson Q&A Discussion Forum')}</h4>
                  <p class="text-xs text-slate-400">${t('qaForumSubtitle', isRtl ? 'سبق کے متعلق کوئی شبہ ہو تو یہاں سوال پوچھیں۔' : 'Ask questions and receive clarifications from instructors and peers.')}</p>
                </div>
                <button onclick="window.Views.openAskQuestionModal('${courseId}', '${activeLesson.id}')" class="btn-primary py-2 px-3.5 text-xs rounded-xl flex items-center gap-1.5">
                  ...
                  <span>${t('askQuestionBtn', isRtl ? '+ نیا سوال پوچھیں' : '+ Ask Question')}</span>
                </button>
              </div>
              ...
              <div class="space-y-3">
                ${lessonQuestions.map(q => `
                  <div class="p-4 bg-slate-950 rounded-2xl border border-slate-800 space-y-3">
                    ...
                    ${q.answer ? `
                      <div class="p-3 bg-slate-900/90 rounded-xl ${isRtl ? 'border-r-4' : 'border-l-4'} border-emerald-500 space-y-1 ${isRtl ? 'text-right' : 'text-left'}">
            ...
            <div id="lms-tab-content-resources" class="p-5 sm:p-6 space-y-4 hidden">
              <div>
                <h4 class="text-sm font-extrabold text-white">${t('downloadableResourcesTitle', isRtl ? 'ڈاؤن لوڈ کے قابل تدریسی مواد' : 'Downloadable Course Resources')}</h4>
                <p class="text-xs text-slate-400">${t('downloadableResourcesSubtitle', isRtl ? 'اس سبق کے متعلقہ خلاصہ جات، پی ڈی ایف اور کتب یہاں سے ڈاؤن لوڈ کریں۔' : 'Download relevant handouts, summaries, and books for this lesson.')}</p>
              </div>
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div class="p-4 bg-slate-950 rounded-2xl border border-slate-800 flex items-center justify-between gap-3">
                  <div class="flex items-center gap-3">
                    ...
                    <div>
                      <div class="text-xs font-bold text-white">${t('summaryAndRulesPdf', isRtl ? 'خلاصۂ سبق و اہم قواعد (PDF)' : 'Lesson Summary & Rules (PDF)')}</div>
                      <div class="text-[10px] text-slate-400 uppercase">PDF • 1.2 MB</div>
                    </div>
                  </div>
                  <a href="#/library" class="btn-primary py-1.5 px-3 text-xs rounded-lg">${t('downloadBtn', isRtl ? 'ڈاؤن لوڈ' : 'Download')}</a>
                </div>
                ...
                <div class="p-4 bg-slate-950 rounded-2xl border border-slate-800 flex items-center justify-between gap-3">
                  <div class="flex items-center gap-3">
                    ...
                    <div>
                      <div class="text-xs font-bold text-white">${t('courseTextbook', isRtl ? 'کتابِ متن و شروحات' : 'Textbook & Commentaries')}</div>
                      <div class="text-[10px] text-slate-400 uppercase">Library Book</div>
                    </div>
                  </div>
                  <a href="#/library" class="btn-secondary py-1.5 px-3 text-xs bg-slate-800 rounded-lg">${t('readOnlineBtn', isRtl ? 'مطالعہ کریں' : 'Read Online')}</a>
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="border-t border-slate-800 bg-slate-900/90 backdrop-blur px-4 sm:px-6 py-4 flex flex-wrap sm:flex-nowrap items-center justify-between gap-3 text-white sticky bottom-0 z-20">
          ${prevLesson ? `
            <a href="#/learn/${course.id}/${prevLesson.id}" class="btn-secondary py-2.5 px-4 text-xs bg-slate-800 text-slate-200 border-slate-700 rounded-xl flex items-center gap-2 hover:bg-slate-700 transition">
              <i data-lucide="${isRtl ? 'chevron-right' : 'chevron-left'}" class="w-4 h-4 shrink-0"></i>
              <span class="truncate max-w-[140px] sm:max-w-[200px]">${t('prevLessonBtn', isRtl ? 'پچھلا سبق:' : 'Previous:')} ${prevLesson.title}</span>
            </a>
          ` : `<div></div>`}
          ...
          ${nextLesson ? `
            <a href="#/learn/${course.id}/${nextLesson.id}" class="btn-primary py-2.5 px-5 text-xs rounded-xl flex items-center gap-2 font-bold shadow-lg shadow-emerald-500/20">
              <span class="truncate max-w-[140px] sm:max-w-[200px]">${t('nextLessonBtn', isRtl ? 'اگلا سبق:' : 'Next:')} ${nextLesson.title}</span>
              <i data-lucide="${isRtl ? 'chevron-left' : 'chevron-right'}" class="w-4 h-4 shrink-0"></i>
            </a>
          ` : `
            <button onclick="window.Views.finishCoursePrompt('${course.id}')" class="btn-primary py-2.5 px-6 text-xs bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-600 hover:to-amber-700 text-slate-950 font-extrabold rounded-xl shadow-xl flex items-center gap-2">
              <i data-lucide="award" class="w-4 h-4"></i>
              <span>${t('finishCourseBtn', isRtl ? 'کورس مکمل کریں اور شاہی سند لیں 🎓' : 'Complete Course & Claim Certificate 🎓')}</span>
            </button>
          `}
        </div>
      </div>
      ...
      <div id="curriculum-sidebar" class="w-full lg:w-[30%] bg-slate-900 border-t lg:border-t-0 ${isRtl ? 'lg:border-r' : 'lg:border-l'} border-slate-800 flex flex-col shrink-0">
        <div class="p-4 sm:p-5 border-b border-slate-800 flex items-center justify-between text-white bg-slate-950/40">
          <div>
            <h4 class="font-extrabold text-sm flex items-center gap-2">
              ...
              <span>${t('curriculumSidebarTitle', isRtl ? 'کورس کا مکمل نصاب' : 'Course Curriculum')}</span>
            </h4>
            <span class="text-xs text-slate-400">${(enrollment.completedLessons || []).length} / ${course.lessons.length} ${t('lessonsCompletedCount', isRtl ? 'اسباق مکمل' : 'Lessons Completed')}</span>
          </div>
              ...
              <a href="#/learn/${course.id}/${lesson.id}" class="p-4 flex items-start gap-3.5 hover:bg-slate-800/80 transition block ${isCurrent ? 'bg-emerald-950/40 border-r-4 border-emerald-500' : ''}">
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <div class="text-[11px] text-slate-400 flex items-center gap-2 mt-1">
                    <span class="capitalize">${lesson.type === 'video' ? t('videoLessonBadge', isRtl ? 'ویڈیو' : 'Video') : lesson.type === 'audio' ? t('audioListen', isRtl ? 'آڈیو' : 'Audio') : t('textReadingBadge', isRtl ? 'مطالعہ' : 'Reading')}</span>
                    <span>•</span>
                    <span>${lesson.durationMinutes || '10'} ${t('durationLabel', isRtl ? 'منٹ' : 'min')}</span>
                  </div>
                </div>
              </a>
            `;
...
window.Views.openTimestampNoteModal = function(courseId, lessonId, lessonTitle) {
  ...
  window.App.showModal(t('addNoteModalTitle', '📝 Add Timestamped Note'), `
    <form onsubmit="window.Views.saveTimestampNote(event, '${courseId}', '${lessonId}')" class="space-y-4 ${fontClass} ${isRtl ? 'text-right' : 'text-left'}" dir="${isRtl ? 'rtl' : 'ltr'}">
      <div>
        <label class="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">${t('lessonTitleField', 'Lesson Title')}</label>
        <input type="text" value="${lessonTitle}" disabled class="input-field text-xs bg-slate-100 dark:bg-slate-800 text-slate-500">
      </div>
      <div>
        <label class="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">${t('timestampField', 'Timestamp (e.g. 02:30)')}</label>
        <input type="text" id="note-timestamp" placeholder="02:30" required class="input-field text-xs text-left" dir="ltr">
      </div>
      <div>
        <label class="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">${t('noteTextField', 'Personal Note or Key Takeaway')}</label>
        <textarea id="note-text" rows="3" placeholder="${t('noteTextPlaceholder', 'Write key rules or important points here...')}" required class="input-field text-xs"></textarea>
      </div>
      <div class="flex justify-end gap-2 pt-2">
        <button type="button" onclick="window.App.closeModal()" class="btn-secondary py-2 px-4 text-xs">${t('btnCancel', 'Cancel')}</button>
        <button type="submit" class="btn-primary py-2 px-5 text-xs font-bold">${t('saveNoteBtn', 'Save Note')}</button>
      </div>
    </form>
  `);
};
...
window.Views.saveTimestampNote = function(event, courseId, lessonId) {
  ...
  window.App.showToast(t('noteSavedToast', 'Note saved successfully! ✓'), 'success');
...
window.Views.deleteTimestampNote = function(courseId, noteId) {
  ...
  localStorage.setItem(notesStorageKey, JSON.stringify(notes));
  window.App.showToast(t('noteDeletedToast', 'Note deleted.'), 'info');
...
window.Views.openAskQuestionModal = function(courseId, lessonId) {
  ...
  window.App.showModal(t('askQuestionModalTitle', '❓ Ask Instructor a Question'), `
    <form onsubmit="window.Views.saveLessonQuestion(event, '${courseId}', '${lessonId}')" class="space-y-4 ${fontClass} ${isRtl ? 'text-right' : 'text-left'}" dir="${isRtl ? 'rtl' : 'ltr'}">
      <div>
        <label class="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">${t('yourQuestionLabel', 'Your Question')}</label>
        <textarea id="qa-question-input" rows="4" placeholder="${t('yourQuestionPlaceholder', 'Write your question clearly regarding this lesson...')}" required class="input-field text-xs"></textarea>
      </div>
      <div class="flex justify-end gap-2 pt-2">
        <button type="button" onclick="window.App.closeModal()" class="btn-secondary py-2 px-4 text-xs">${t('btnCancel', 'Cancel')}</button>
        <button type="submit" class="btn-primary py-2 px-5 text-xs font-bold">${t('submitQuestionBtn', 'Submit Question')}</button>
      </div>
    </form>
  `);
};
...
window.Views.saveLessonQuestion = function(event, courseId, lessonId) {
  ...
  window.App.closeModal();
  window.App.showToast(t('questionSubmittedToast', 'Your question has been submitted successfully! ✓'), 'success');
  window.Views.renderLearningPlayer({ courseId: courseId, lessonId: lessonId });
};
...
window.Views.toggleLessonCompletion = async function(courseId, lessonId, isCompleted) {
  ...
  if (updated && updated.progressPercentage === 100 && isCompleted) {
    ...
    const newCert = {
      ...
      userName: user.name,
      courseId: courseId,
      title: course ? course.title : (isRtl ? 'کامیاب تکمیلِ کورس' : 'Course Completion'),
      grade: isRtl ? 'ممتاز (Distinction)' : 'Distinction (Mumtaz)',
    ...
    window.App.showModal(t('courseCompletedModalTitle', '🎉 Congratulations! Course Completed!'), `
      <div class="text-center space-y-4 py-4 ${fontClass}" dir="${isRtl ? 'rtl' : 'ltr'}">
        ...
        <h3 class="text-2xl font-extrabold text-slate-900 dark:text-white">${isRtl ? 'مبارک ہو، محترم' : 'Congratulations,'} ${user.name}!</h3>
        <p class="text-xs sm:text-sm text-slate-600 dark:text-slate-400 max-w-md mx-auto leading-relaxed">
          ${t('courseCompletedModalSubtitle', isRtl ? 'آپ نے اس کورس کا تمام نصاب کامیابی اور فضیلت کے ساتھ 100% مکمل کر لیا ہے۔ آپ کی باضابطہ تصدیق شدہ شاہی سند جاری کر دی گئی ہے۔' : 'You have completed 100% of this course curriculum with excellence. Your official verified certificate has been issued.')}
        </p>
        <div class="pt-3 flex flex-wrap justify-center gap-3">
          <a href="#/certificates" onclick="window.App.closeModal()" class="btn-primary py-2.5 px-6 text-xs bg-amber-500 hover:bg-amber-600 text-slate-950 font-extrabold border-none shadow-lg">${t('viewPrintCertificateBtn', isRtl ? 'اپنی شاہی سند دیکھیں و پرنٹ کریں' : 'View & Print Certificate')}</a>
          <button onclick="window.App.closeModal()" class="btn-secondary py-2.5 px-4 text-xs">${t('closeBtn', 'Close')}</button>
        </div>
      </div>
    `);
  }
...
window.Views.finishCoursePrompt = function(courseId) {
  const t = (k, f) => window.I18N ? window.I18N.t(k, f) : f;
  window.App.showToast(t('msgSuccess', 'Mashallah! Your certificate is available in the certificates section.'), 'success');
